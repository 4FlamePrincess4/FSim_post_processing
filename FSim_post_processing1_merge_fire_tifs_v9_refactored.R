...
#read_and_combine_rasters <- function(file_paths) {
#  raster_list <- lapply(file_paths, terra::rast)
...
process_single_fire_season <- function(each_season, this_season_fireIDs, this_season_AD_stack, this_season_FL_stack) {
  ...
  #this_season_AD_stack <- read_and_combine_rasters(this_season_AD_file)
  #unlink(this_season_AD_file)
  #rm(this_season_AD_file)
...
merge_and_write_rasters <- function(each_season, this_season_fireIDs, this_season_AD_stack, this_season_FL_stack) {
...
process_overlaps <- function(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel){
  library(RSQLite)
  ...
  overlapping_ig <- tidyterra::as_sf(terra::vect(x = overlapping_ig, crs=ref_sys$srtext))
  ...
  rm(overlapping_ig, season_fire_perims)
  ...
  for(i in seq_along(unique_overlapping_fire_indices)){
    ...
    this_season_AD_stack[[this_index]] <- earliest_arrival_AD_stack[[i]]
    this_season_FL_stack[[this_index]] <- earliest_arrival_FL_stack[[i]]
    rm(this_index)
  ...
  rm(earliest_arrival_AD_stack, earliest_arrival_FL_stack, fires_to_delete)
  #Finally, export the resulting edited tifs in a 3-band season raster stack
  merge_and_write_rasters(each_season, this_season_fireIDs, this_season_AD_stack, this_season_FL_stack)
}
handle_more_than_two_overlaps <- function(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel) {
  ...
  for(i in seq_along(cells_over_two_overlap)){
    ...
    # Convert the data frame to a vector & find the indices of non-NA values
    non_na_indices <- which(!is.na(as.vector(t(arrival_days_df[1,]))))
  ...
  rm(excess_overlap_fires, non_na_indices, excess_overlap_fireids)
  print(paste("These are the excess fires:", paste(excess_fires, collapse = ", ")))
  #Now process the overlapping fires
  process_overlaps(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel)
}


handle_two_or_fewer_overlaps <- function(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel) {
...
process_overlapping_fires <- function(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices) {
  ...
  if(max_overlapping_fires > 2) {
    rm(max_overlapping_fires)
    handle_more_than_two_overlaps(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel)
  } else { #If there are only two fires at any given pixel, use the handle_two_or_fewer_overlaps function.
    rm(max_overlapping_fires)
    handle_two_or_fewer_overlaps(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel)
  }
}
...
process_fire_season <- function(each_season) {
  ...
  #this_season_AD_file <- lapply(this_season_AD_filenames, function(filename) {
  ...
  #this_season_FL_file <- lapply(this_season_FL_filenames, function(filename) {
  #  aligned_raster <- align_and_stack_tifs(filename, foa_lcp = foa_lcp)
  #  temp_filename <- tempfile(fileext = ".tif")
  #  terra::writeRaster(aligned_raster, temp_filename, overwrite = TRUE)
  #  rm(aligned_raster)
  ...
  rm(this_season_scen, this_season_foa_run, this_season_AD_filenames)
  ...
  if(length(this_season_FL_filenames) <= 1){
    ...
    if(length(overlap_indices) == 0){
      ...
      process_overlapping_fires(each_season, this_season_fireIDs, this_season_pt, this_season_AD_stack, this_season_FL_stack, season_fire_perims, ref_sys, overlap_indices)
    }
  } 
}
...
#Let's try setting up a cluster and using that with future_map()
...
future_options <- furrr_options(globals=c("wd", "firelists", "foa_run", "scenario", "foa_lcp", "logger", "read_and_combine_rasters",
