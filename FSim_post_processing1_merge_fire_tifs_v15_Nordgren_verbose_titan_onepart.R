...
###############################################
#These variables are based on your file naming convention, so please adjust accordingly. 
foa_run <- "FOA1c_r7"
#If you have a scenario, edit lines 37, 48-50, 153-164, 195-197, 254-256, 288-290, 338-349, 
# 374-378, 426-430, 457, 465-466, and 493-502. You'll only need to do this once, assuming your naming conventions are consistent.
#scenario <- ""

#You need the below variables regardless of naming convention. Adjust the foa lcp path accordingly.
foa_lcp_path <- "./_inputs/lcp/FOA1c_LCG_LF2022_FBFM40_230_120m.tif"
number_of_seasons <- 20000

#STEP 2: Read in firelist files 
###############################################
#Read in run fire lists
firelists <- read_csv(paste0("./", foa_run, "_FireSizeList.csv"))
#Use the below code if you have a scenario
#firelists <- read_csv(paste0("./", foa_run, "_", scenario, "_FireSizeList.csv"))
...
process_single_fire_season <- function(each_season, this_season_fireIDs, this_season_foa_run) {
...
process_overlaps <- function(each_season, this_season_fireIDs, this_season_foa_run, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df){
  library(RSQLite)
  ...
  con <- RSQLite::dbConnect(RSQLite::SQLite(), dbname = paste0(wd,"/", foa_run, "_Ignitions.sqlite"))
  #Decomment the below if you have a scenario
  #con <- RSQLite::dbConnect(RSQLite::SQLite(), dbname = paste0(wd,"/", foa_run, "_",
  #                                                             scenario, "_Ignitions.sqlite"))
  ...
  for(pairs in 1:nrow(overlapping_fire_ids_df)){
    ...
    if(first_ignition_in_second_perim){
      ...
      this_perim_AD_raster <- terra::rast(paste0(wd, "/", this_season_foa_run, "_ArrivalDays/",
                                   "FireID_", second_ID, ".tif"))
      #Decomment the below if you have a scenario
      #this_perim_AD_raster <- terra::rast(paste0(wd, "/", this_season_foa_run, "_", scenario,"_ArrivalDays/",
      #                             "FireID_", second_ID, ".tif"))
   #Use this info to read in the flame length tif filenames for this season's fires
...
handle_more_than_two_overlaps <- function(each_season, this_season_fireIDs, this_season_foa_run, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel, max_overlapping_fires) {
  ...
  process_overlaps(each_season, this_season_fireIDs, this_season_foa_run, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df)
}


handle_two_or_fewer_overlaps <- function(each_season, this_season_fireIDs, this_season_foa_run, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df) {
...
process_overlapping_fires <- function(each_season, this_season_fireIDs, this_season_foa_run, season_fire_perims, ref_sys, overlap_indices) {
  ...
  if(max_overlapping_fires > 2) {
    handle_more_than_two_overlaps(each_season, this_season_fireIDs, this_season_foa_run, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df, num_non_na_per_pixel, max_overlapping_fires)
  } else { #If there are only two fires at any given pixel, use the handle_two_or_fewer_overlaps function.
    handle_two_or_fewer_overlaps(each_season, this_season_fireIDs, this_season_foa_run, season_fire_perims, ref_sys, overlap_indices, overlapping_fire_ids_df, overlapping_fire_indices_df)
  }
}
...
process_fire_season <- function(each_season) {
  ...
  #this_season_scen <- rep(scenario, length(this_season_fireIDs))
  ...
  if(length(this_season_fireIDs) == 1){
    process_single_fire_season(each_season, this_season_fireIDs, this_season_foa_run)
