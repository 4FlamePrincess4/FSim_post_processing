...
calc_prob_w_accumulator <- function(season_fire_path, categories, foa_lcp, accum_bp, fl_accumulators){
...
# Remember arguments must be in this order: season_fire_path, categories, foa_lcp, accum_bp, fl_accumulators
...
for(season in 1:length(season_fire_files)){
  ...
  result <- calc_prob_w_accumulator(season_fire_path, categories, foa_lcp, accum_bp, fl_accumulators)
}
